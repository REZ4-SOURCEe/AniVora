...
function showPage(id, skipHistory) {
  // اگه همون صفحه فعلی بود، کاری نکن
    ...
    <div class="anime-card" onclick="showPage('detail')">
      <div class="card-poster">
        ...
        <div class="card-poster-overlay">
          <button class="card-watch-btn" onclick="event.stopPropagation();showPage('watch')">Watch</button>
...
function toggleFullscreen(e) {
  ...
  if (!isFs) {
    const req = playerWrap.requestFullscreen || playerWrap.webkitRequestFullscreen || playerWrap.msRequestFullscreen;
    if (req) {
      req.call(playerWrap).catch(err => console.log('Fullscreen error:', err));
    }
...
============================================ */
function init() {
  // ثبت صفحه اول در history
  const startPage = (location.hash ? location.hash.replace('#','') : 'home');
  const validPages = ['home','explore','detail','watch','seasonal','watchlist','profile','calendar','login','admin'];
  ...
  // اگه صفحه اول از hash خونده شد، همون رو نشون بده
  if (initialPage !== 'home') {
    document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));
